...
const FILTERS: Record<AssetType, { name: string; extensions: string[] }[]> = {
    ...
    texture: [{ name: 'Texture Files', extensions: ['dds', 'DDS', 'ktx2', 'cfg', 'json'] }],
};
    ...
    if (selected.length === 0) {
        return (
            <div className={styles.empty}>
                ...
                <p className={styles.emptySub}>Files are routed to the correct folder automatically based on FS20/FS24 structure</p>
            </div>
        );
    }

    const countLabel = `${selected.length} ${selected.length === 1 ? 'livery' : 'liveries'} selected`;

    return (
        <div className={styles.panel}>
            <div className={styles.panelHeader}>
                <span className={styles.selectedNote}>{countLabel}</span>
            </div>
            ...
            <div className={styles.sections}>
                <div className={styles.section}>
                    ...
                    <div className={styles.sectionBody}>
                        ...
                        <div className={styles.fileRow}>
                            <button className={styles.browseBtn} onClick={() => browse('manager-thumbnail')}>Browse…</button>
                            {managerFile ? (
                                <>
                                    <span className={styles.fileName} title={managerFile}>{basename(managerFile)}</span>
                                    <button className={styles.clearBtn} onClick={() => setManagerFile(null)}>×</button>
                                </>
                            ) : (
                                <span className={styles.filePlaceholder}>No file selected</span>
                            )}
                            <button className={styles.copyBtn} onClick={() => copyAssets('manager-thumbnail')} disabled={!canCopy('manager-thumbnail')}>
                ...
                <div className={styles.section}>
                    ...
                    <div className={styles.sectionBody}>
                        ...
                        <div className={styles.fileRow}>
                            <button className={styles.browseBtn} onClick={() => browse('ingame-thumbnail')}>Browse…</button>
                            {thumbnailFiles.length > 0 && <button className={styles.clearBtn} onClick={() => setThumbnailFiles([])}>Clear</button>}
                            {thumbnailFiles.length === 0 && <span className={styles.filePlaceholder}>No files selected</span>}
                            <button className={styles.copyBtn} onClick={() => copyAssets('ingame-thumbnail')} disabled={!canCopy('ingame-thumbnail')}>
                ...
                <div className={styles.section}>
                    <div className={styles.sectionHeader}>
                        ...
                        <span className={styles.acceptTag}>DDS · KTX2 · cfg · json</span>
                    </div>
                    <div className={styles.sectionBody}>
                        <p className={styles.sectionDesc}>
                        ...
                        <div className={styles.fileRow}>
                            <button className={styles.browseBtn} onClick={() => browse('texture')}>Browse…</button>
                            {textureFiles.length > 0 && <button className={styles.clearBtn} onClick={() => setTextureFiles([])}>Clear</button>}
                            {textureFiles.length === 0 && <span className={styles.filePlaceholder}>No files selected</span>}
                            <button className={styles.copyBtn} onClick={() => copyAssets('texture')} disabled={!canCopy('texture')}>
